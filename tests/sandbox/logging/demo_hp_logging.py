...
if __name__ == "__main__":
    logger = get_async_logger("main_app")

    logger.info("Ứng dụng khởi động")
    try:
        # Mô phỏng tác vụ
        for i in range(100):
            logger.debug(f"Xử lý nội bộ {i}")  # Không được ghi do level INFO
            logger.info(f"Đang xử lý tác vụ {i}")

            if i % 10 == 0:
                logger.warning(f"Checkpoint tại {i}")
